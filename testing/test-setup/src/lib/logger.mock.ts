...
beforeAll(async () => {
  const { logger }: typeof import('@push-based/zod2nx-schema') =
    await vi.importActual('@push-based/zod2nx-schema');
...
vi.mock('ora', async () => {
  const oraDefault = await vi.importActual<typeof import('ora').default>('ora');
  ...
  const mockOra = (options: string | import('ora').Options | undefined) => {
    ...
    vi.spyOn(spinner, 'succeed').mockImplementation((text?: string) =>
      spinner.stopAndPersist({ text, symbol: ansis.green('✔') }),
    );
    vi.spyOn(spinner, 'fail').mockImplementation((text?: string) =>
